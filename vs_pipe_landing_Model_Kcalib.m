...
if camera_params_est
    ind_act = [ind_act, 10,11,12,13];
end
l_ind_act = length(ind_act);

%Camera
T = 1/25; %Camera frame rate
K = [630, 0.0, 360; 
     0.0, 630, 240; 
     0.0, 0.0, 1.0];
if camera_params_est
...
%FoV = [-1 1 -1 1]; % Camera field of view (normalized image plane)
%K = eye(3);
invK = inv(K);
...
i=1;
P(i).radius = 6 * 0.0254; % Radius in inches, converted to m
P(i).length = pipe_length; % Pipe length [m]
P(i).v = iy; % Central unit versor
P(i).p = [P(i).radius;0;P(i).radius]; % A point on the central axis
P(i).R = [ix cross(P(i).v,ix) P(i).v];
i=i+1;
P(i).radius = 8 * 0.0254; % Radius in inches, converted to m
P(i).length = pipe_length; % Pipe length [m]
P(i).v = iy; % Central unit versor
...
% i=i+1;
% P(i).radius = 8 * 0.0254; % Radius in inches, converted to m
% P(i).length = pipe_length/2; % Pipe length [m]
% P(i).v = ix; % Central unit vector
% P(i).p = [3*P(i).radius-P(i).length/2; P(i).length; P(i).radius]; % A point on the central axis
% P(i).R = [iy cross(iy,P(i).v) P(i).v];
...
while t < tf+2
    ...
    ylabel('y [m]');
    zlabel('z [m]');
    %title(sprintf('Pipe-rack (time %.3f s)', t));


    %% CAMERA Motion

    cam_traj_angle = 4*pi*min(t,tf)/tf; %!!!
    cam_p = [0.5; pipe_length/2; 2.5] + [0.25*cos(cam_traj_angle);0.5*sin(0.5*cam_traj_angle);0.25*sin(cam_traj_angle)];
    ...
    for i=1:np
        ...
        for j=1:size(P(i).RP,1)
            r = Pc*[P(i).RP(j,:)';1];%cam_R'*(P(i).RP(j,:)'-cam_p);
            m = [r(1)/r(3); r(2)/r(3)];
            if IsInFoV(m,FoV)
                plot(m(1),m(2),'b*');
            end
    ...
    %Simulate the estimate of the camera pose
    %cam_pe = cam_est_tau * cam_pe + (1-cam_est_tau)*cam_p;
    %cam_Re = cam_R;
